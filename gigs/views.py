...
def validate_ticket_price(request):
    ...
    # Get the user's performance tier
    try:
        artist = request.user.artist
        tier = artist.performance_tier
        
        # Check if price meets minimum requirement for the tier
        if price < tier.min_ticket_price:
            return Response({
                "valid": False,
                "error": f"Price must be at least ${tier.min_ticket_price} for your tier"
            }, status=status.HTTP_400_BAD_REQUEST)
            
        return Response({
            "valid": True,
            "min_price": tier.min_ticket_price,
            "message": "Price is valid for your performance tier"
        })
        ...
        logger.error(f"Error validating ticket price: {str(e)}", exc_info=True)
        return Response(
            {"error": "An error occurred while validating the price"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
